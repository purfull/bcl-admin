...
import { ResponsiveBranchDataTable } from "./CustomerData";
import Alert from "../alert/Alert";
// import ParentComponent from "./CustomerData";
...
const Customer = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_URL}/dashboards/customer?isActive=${isActive}`, {
      method: 'GET',
      headers: {
  ...
  const handleConfirmDelete = () => {
    ...
    fetch(`${import.meta.env.VITE_URL}/customer/${path}`, {
      method: 'PUT', // Ensure this is the correct method for your API
      headers: {
        'Content-Type': 'application/json',
      },
      signal: abortController.signal,
      body: JSON.stringify({ BranchCode: rowToDelete.row.Code }) // Ensure the payload is correctly formatted
    })
      .then(result => result.json())
      .then(response => {
