...
import EditBranch from "./EditPaymode";
import '../org/org.css';
import { ResponsiveBranchDataTable } from "./PaymodeData";
...
const Branch = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://hmsapi.appxes-erp.in/Paymode/GetAllPaymode`,{
      method: 'GET',
      headers: {
  ...
  useEffect(() => {
    if (data.length > 0) {
      setListData(data.map(el => ({
        PaymodeID: el.PaymodeID,
        PaymodeCode: el.PaymodeCode,
        PaymodeName: el.PaymodeName
      })));
    }
  }, [data]);
