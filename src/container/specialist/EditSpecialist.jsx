...
const EditSpecialist = ({ row, onCancel }) => {
  const [editData, setEditData] = useState({});
  const [activeTab, setActiveTab] = useState('Specialist Info'); // Default active tab
  ...
  useEffect(() => {
    ...
    setServicesOffered(initialServices);
  }, []);


  useEffect(() => {
    const initialSpecialists = [
      { id: 1, businessId: 'B001', businessName: 'Coffee Shop', requestId: 'REQ001', status: 'Active' },
      { id: 2, businessId: 'B002', businessName: 'Bookstore', requestId: 'REQ002', status: 'Pending' },
      { id: 3, businessId: 'B003', businessName: 'Flower Shop', requestId: 'REQ003', status: 'Active' },
      { id: 4, businessId: 'B004', businessName: 'Gym', requestId: 'REQ004', status: 'Inactive' },
      { id: 5, businessId: 'B005', businessName: 'Bakery', requestId: 'REQ005', status: 'Active' },
    ];
  
    setBusinessDetails(initialSpecialists);
  }, []);
  ...
  return (
    <div>
      <div className="mb-4 border-b border-gray-200 dark:border-gray-700">
        <ul className="flex flex-wrap -mb-px text-sm font-medium text-center" role="tablist">
          <li className="me-2" role="presentation">
            <button
              className={`inline-block p-4 border-b-2 rounded-t-lg ${activeTab === 'Specialist Info' ? 'border-purple-600 text-purple-600' : 'hover:text-gray-600'}`}
              onClick={() => setActiveTab('Specialist Info')}
              type="button"
            >
              Specialist Info
            </button>
          ...
          <li className="me-2" role="presentation">
            <button
              className={`inline-block p-4 border-b-2 rounded-t-lg ${activeTab === 'Services Offered' ? 'border-purple-600 text-purple-600' : 'hover:text-gray-600'}`}
          ...
          <li className="me-2" role="presentation">
            <button
              className={`inline-block p-4 border-b-2 rounded-t-lg ${activeTab === 'Business Details' ? 'border-purple-600 text-purple-600' : 'hover:text-gray-600'}`}
      ...
      <div id="default-styled-tab-content">
        {activeTab === 'Specialist Info' && (
          <form onSubmit={handleSubmitBusinessInfo}>
            {/* Business Info form fields here */}
            <div>
            <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center justify-start">
                    <label htmlFor="specialistName" className="w-[30%] font-medium">Specialist Name</label>
                    <div className="w-[70%]">
                    <input type="text" className="form-control" id="specialistName" onChange={handleChange} />
                    </div>
                </div>
                <div className="flex items-center justify-start">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div className="flex justify-end items-center">
                  <label htmlFor="businessPlace" className="font-medium mr-[1vw]">Specialist Place</label>
                  <label className="switch">
