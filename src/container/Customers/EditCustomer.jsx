...
const EditCustomer = ({row,  onCancel }) => {
  ...
  const req = row.newCustomer ? 'POST' : 'PUT'
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_URL}/branch/edit`, {
      ...
      body: JSON.stringify({ CustomerCode: row.CustomerCode })
    })
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center justify-start">
            <label htmlFor="OrgId" className="w-[30%] font-medium">Customer ID</label>
            <div className="w-[70%]">
              <input type="text" className="form-control" id="OrgId" defaultValue={editData.CustomerCode || ''} onChange={handleChange} disabled />
            </div>
          </div>
          <div className="flex items-center justify-start">
            <label htmlFor="ORG_Name" className="w-[30%] font-medium ">Name</label>
            <div className="w-[70%]">
              <input type="text" className="form-control" id="ORG_Name" defaultValue={editData.CustomerName || ''} onChange={handleChange} />
            </div>
          </div>
          <div className="flex items-center justify-start">
